...
public class PlayerManager : MonoBehaviour
{
    ...
    private enum MovementState { idle, running, jumping, falling, crouching, crouchwalk, attack, crouchAttack, takeHit, deadth, slide, turnAround }
    ...
    [Header("Attack")]
    private float attackStateTransitionTime = 1f;
    public float attackStateCurrent = -1;
    ...
    {   
        ...
        if (isTurnAround)
        {
            state = MovementState.turnAround;
    ...
    private void Attacking()
    {
        ...
        if(Input.GetKeyDown(KeyCode.C))
        {
            ...
            if (attackStateCurrent >= 3)
            {
                attackStateCurrent = 0;
            }

            attackState = (AttackState)attackStateCurrent;
        }
    }
    ...
    public void TakeHit()
    {
        ...
        //Canvas
        numberOfHeath--;
        playerLifeCanvas.HiddenHeatSegment(numberOfHeath);
        takeHit = true;
    }
    ...
    private void EndOfFrameTurnAround()
    {
        isTurnAround = false;
    }
        ...
        if(colliders.Length > 0)
        {
            foreach (Collider2D collider in colliders)
            {
                if(collider.CompareTag("Enemy"))
                {
                    ...
                    enemy.TakeHit(direction, pushoutValue);
                }
            }
        }
    }
